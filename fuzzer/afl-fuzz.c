...
static void update_input_structure(u8* fname, struct queue_entry* q) {
  ...
  if (model_type == MODEL_PEACH) {
    ...
    if (pipe(pipefd) < 0) {
			PFATAL("NFuzzer cannot create a pipe to communicate with Peach");
      exit(1);
    }
    ...
    /// NOT SUPPORTED
    PFATAL("NFuzzer currently only supports Peach models! Please use -w peach option");
  }
...
static void show_stats(void) {
  ...
  if (term_too_small) {
    ...
    SAYF(cBRI "Your terminal is too small to display the UI.\n"
         "Please resize terminal window to at least 70x24.\n" cRST);
...
#define SP20    SP10 SP10
  ...
  SAYF(SET_G1 bSTG bLT bH bSTOP cCYA " 运行过程 " bSTG bH30 bH5 bH2 bH2 bHB
       bH bSTOP cCYA " 结果总览 " bSTG bH5 bH bRT "\n");

  SAYF(bV bSTOP SP20 SP20 SP10 bSTG bV bSTOP SP10 SP5 SP SP bSTG bV "\n");
  ...
  SAYF(bV bSTOP "       运行时间: " cRST "%-32s " bSTG bV bSTOP
       " 执行轮次: %s%-4s  " bSTG bV "\n",
  ...
  if (!dumb_mode && (last_path_time || resuming_fuzz || queue_cycle == 1 ||
    ...
    SAYF(bV bSTOP " 上一条全新路径: " cRST "%-32s ",
    ...
    if (dumb_mode)
      ...
      SAYF(bV bSTOP " 上一条全新路径: " cPIN "n/a" cRST
           " (现为非插桩模式)          ");

     else

      SAYF(bV bSTOP " 上一条全新路径: " cRST "none yet " cLRD
           "(请检查语法结构)        ");

  }

  SAYF(bSTG bV bSTOP " 路径总数: " cRST "%-4s  " bSTG bV "\n",
       DI(queued_paths));
  ...
  SAYF(bV bSTOP " 上一次独特出错: " cRST "%-32s " bSTG bV bSTOP
       " 独特出错: %s%-5s " bSTG bV "\n",
  ...
  SAYF(bV bSTOP " 上一次独特挂起: " cRST "%-32s " bSTG bV bSTOP
       " 独特挂起: " cRST "%-5s " bSTG bV "\n",
       DTD(cur_ms, last_hang_time), tmp);
      
  SAYF(bV bSTOP SP20 SP20 SP10 bSTG bV bSTOP SP10 SP5 SP SP bSTG bV "\n");

  SAYF(bVR bH bSTOP cCYA " 轮次进展 " bSTG bH10 bH10 bH2 bHB bH bSTOP cCYA
       " 覆盖位图 " bSTG bH5 bHT bH10 bH5 bH2 bVL "\n");

  SAYF(bV bSTOP SP20 SP10 SP SP SP bSTG bV bSTOP SP20 SP10 SP SP SP SP bSTG bV "\n");
  ...
  SAYF(bV bSTOP " 正在处理: " cRST "%-21s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%0.02f%% / %0.02f%%", ((double)queue_cur->bitmap_size) *
          100 / MAP_SIZE, t_byte_ratio);

  SAYF(" 位图密度: %s%-22s " bSTG bV "\n", t_byte_ratio > 70 ? cLRD :
       ((t_bytes < 200 && !dumb_mode) ? cPIN : cRST), tmp);

  sprintf(tmp, "%s (%0.02f%%)", DI(cur_skipped_paths),
          ((double)cur_skipped_paths * 100) / queued_paths);

  SAYF(bV bSTOP " 超时路径: " cRST "%-21s " bSTG bV, tmp);

  sprintf(tmp, "%0.02f 比特/元组",
          t_bytes ? (((double)t_bits) / t_bytes) : 0);

  SAYF(bSTOP " 覆盖计数: " cRST "%-26s " bSTG bV "\n", tmp);

  SAYF(bV bSTOP SP20 SP10 SP SP SP bSTG bV bSTOP SP20 SP10 SP SP SP SP bSTG bV "\n");

  SAYF(bVR bH bSTOP cCYA " 阶段进展 " bSTG bH20 bH2 bX bH bSTOP cCYA
       " 深入结果 " bSTG bH20 bH2 bH bVL "\n");
  ...
  SAYF(bV bSTOP " 变异方式: " cRST "%-21s " bSTG bV bSTOP
       " 偏好路径: " cRST "%-22s " bSTG bV "\n", stage_name, tmp);
  ...
  SAYF(bV bSTOP " 执行阶段: " cRST "%-21s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s (%0.02f%%)", DI(queued_with_cov),
          ((double)queued_with_cov) * 100 / queued_paths);

  SAYF(" 新的覆盖: " cRST "%-22s " bSTG bV "\n", tmp);

  sprintf(tmp, "%s (%s%s 独特)", DI(total_crashes), DI(unique_crashes),
          (unique_crashes >= KEEP_UNIQUE_CRASH) ? "+" : "");

  if (crash_mode) {

    SAYF(bV bSTOP " 总执行数: " cRST "%-21s " bSTG bV bSTOP
         "   新的出错: %s%-24s " bSTG bV "\n", DI(total_execs),
  ...
  SAYF(bV bSTOP " 执行速度: " cRST "%-21s ", tmp);

  sprintf(tmp, "%s (%s%s 独特)", DI(total_tmouts), DI(unique_tmouts),
          (unique_hangs >= KEEP_UNIQUE_HANG) ? "+" : "");

  SAYF (bSTG bV bSTOP " 总超时数: " cRST "%-24s " bSTG bV "\n", tmp);

  SAYF(bV bSTOP SP20 SP10 SP SP SP bSTG bV bSTOP SP20 SP10 SP SP SP SP bSTG bV "\n");

  SAYF(bLB bH30 bH2 bH bHT bH20 bH10 bH2 bH2 bRB bSTOP cRST RESET_G1 "\n"); 
  ...
  if (cpu_core_count) {
    ...
    if (cpu_aff >= 0) {
      ...
      SAYF(SP20 SP20 SP10 SP5 cGRA "[cpu%03u:%s%3u%%" cGRA "]\r" cRST,
           MIN(cpu_aff, 999), cpu_color,
           MIN(cur_utilization, 999));

    } else {

      SAYF(SP20 SP20 SP10 SP5 cGRA "   [cpu:%s%3u%%" cGRA "]\r" cRST,
...
struct chunk *get_chunk_to_delete(struct chunk **chunks_array, u32 total_chunks,
  ...
  *del_len = 0;
  ...
  for (i = 0; i < 3; ++i) {
    ...
    /* 可能是起始字节或结束字节未知（具有负值），因此我们实际上只在这些边界已知时执行删除操作。 */
    if (chunk_to_delete->modifiable && start_byte >= 0 &&
        chunk_to_delete->end_byte >= start_byte) {
      /* 此处要删除的长度比end_byte-start_byte多1个，因为end - byte正好是最后一个字节的位置,因此需要加1 */
      *del_from = start_byte;
 ...
 * Parameters:
 *
 * temp_len:  out_buf缓冲区的长度.
 *
 * out_buf: The output 缓冲区.
 */
...
static void check_term_size(void) {
  ...
  if (ws.ws_row < 24 || ws.ws_col < 70) term_too_small = 1;
